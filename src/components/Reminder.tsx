  ...
  const content = !ok ? (
    ...
    <div className="flex-none">
      <div className="dropdown dropdown-end">
        <div tabIndex={0} className="btn btn-ghost btn-md">
          Reminder
        </div>

        <div
          tabIndex={0}
          className="mt-3 z-[1] card  dropdown-content w-max bg-base-100 shadow bg-accent"
        >
...
async function ReminderList({ data }: { data: Reminder[] }) {
  const content =
    data.length &&
    data.map((reminder: Reminder) => {
      const slotData = {
        shortMessage:
          reminder.book_title + "last due date:" + reminder.last_date,
...
function ReminderSlot({ data }: { data: Slot }) {
  return (
    <div
      tabIndex={0}
      className="collapse bg-primary text-primary-content focus:bg-secondary focus:text-secondary-content m-1"
    >
